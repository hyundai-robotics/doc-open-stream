...
해당 기능은 ${cond_model} Open API를 짧은 주기로 반복 호출하여,  
클라이언트가 그 결과를 스트리밍 형태로 지속적으로 수신할 수 있도록 제공되는 인터페이스입니다.

<br>

${cond_model} 제어기 내부의 TCP 기반의 경량 서버를 통해  
...
CONTROL 명령이란? 클라이언트가 ${cond_model} Open API를 통해 단발성 제어 요청을 전달하기 위한 명령입니다.  
클라이언트가 필요에 따라 짧은 주기로 반복 송신할 수 있으며, 정상적으로 성공한 경우에는 응답을 반환하지 않고,  
오류가 발생한 경우에만 오류 응답을 반환합니다.
